// store.js — 全局状态 + localStorage 持久化 (P1)
// 一个极简的外部 store（subscribe/get/set）+ React useSyncExternalStore 绑定。
// 所有页面共享同一份数据：API Key、灵感罐素材、收藏、跨页传递的灵感等。
...
const Store = (() => {
  ...
  let state = {
    // Key 优先级：本地配置文件 config.local.js > localStorage > 空（触发弹窗）
    apiKey: (typeof window !== 'undefined' && window.__ZHIPU_KEY__) || load('apiKey', ''),
    keyFromConfig: !!(typeof window !== 'undefined' && window.__ZHIPU_KEY__),
