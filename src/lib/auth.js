...
export const Auth = {
  ...
  async signUp(email, password, name) {
    ...
    if (hasSupabase) {
      // emailRedirectTo：确认邮件点完后跳回「当前 App 所在地址」（本地 localhost:5173 / 线上 Vercel 域名都自适应），
      // 而不是依赖 Supabase 后台写死的 Site URL。注意：该地址必须在 Supabase → Auth → URL Configuration 的 Redirect URLs 白名单里。
